defmodule Timemanager.Workingtimes do
  ...
  # get workingtime by id and by user id (user id is not the same as the user name)
  ...
  def plop(user, start, endtime) do
    Repo.all(from w in Workingtime,
      where: w.user == ^user and fragment("?::text LIKE ?::text", w.start, ^"%#{start}%") and fragment("?::text LIKE ?::text", w.end, ^"%#{endtime}%"))
  end


  def get_workingtime_by_user(id, user) do
     Repo.all(from w in Workingtime, where: w.id == ^id and w.user == ^user)
  end
